...
async def set_recentparams(List):
    try:
        ...
        result = {
            ...
            "reTime": time.strftime('%Y-%m-%d %H:%M:%S',
                                    time.localtime(int(abs(List['shipData'][0]['recordDateTime'] / 1000)))),
...
async def set_shipRecentparams(List):
    try:
        damageColor = await set_damageColor(List['shipData'][0]['shipInfo']['shipInfo']['shipType'],
                                            List['pvpInfo']['damage'])
        winsColor = await set_winColor(List['pvpInfo']['wins'])
        solo_winsColor = await set_winColor(List['pvpSoloInfo']['wins'])
        solo_damageColor = await set_damageColor(List['shipData'][0]['shipInfo']['shipInfo']['shipType'],
                                                 List['pvpSoloInfo']['damage'])
        div2_winsColor = await set_winColor(List['pvpTwoInfo']['wins'])
        div2_damageColor = await set_damageColor(List['shipData'][0]['shipInfo']['shipInfo']['shipType'],
                                                 List['pvpTwoInfo']['damage'])
        div3_winsColor = await set_winColor(List['pvpThreeInfo']['wins'])
        div3_damageColor = await set_damageColor(List['shipData'][0]['shipInfo']['shipInfo']['shipType'],
                                                 List['pvpThreeInfo']['damage'])
        rank_winsColor = await set_winColor(List['rankInfo']['wins'])
        rank_damageColor = await set_damageColor(List['shipData'][0]['shipInfo']['shipInfo']['shipType'],
                                                 List['rankInfo']['damage'])
